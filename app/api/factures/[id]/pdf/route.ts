// ============================================================================
// API ROUTE — GET /api/factures/:id/pdf
// P6, lot Factures. Génère le PDF de la facture via pdf-lib (D5). Même
// patron que /api/devis/[id]/pdf/route.ts.
// ============================================================================
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    drawText(page, height, helveticaBold, "FACTURE", width - 168, 53, 12, white);
    drawText(page, height, helvetica, `No ${factureRow.reference}`, width - 175, 80, 10, white);
    drawText(
      page,
      height,
      helvetica,
      new Date(factureRow.created_at).toLocaleDateString("fr-FR"),
      width - 175,
      95,
      9,
      rgb(0.7, 0.75, 0.85)
    ...
    cursorY += 18;
    drawText(page, height, helveticaBold, factureRow.demandes?.nom_complet || "-", 50, cursorY, 13, nexusBlue);
    cursorY += 16;
    drawText(page, height, helvetica, factureRow.demandes?.service || "-", 50, cursorY, 10, grayDark);
    cursorY += 30;
    ...
    const rowHeight = 22;
    const nbRows = factureRow.facture_lignes.length;
    ...
    const sortedLignes = [...factureRow.facture_lignes].sort((a, b) => a.ordre - b.ordre);
    for (const ligne of sortedLignes) {
      ...
      drawText(page, height, helvetica, formatMoney(ligne.unit_price, factureRow.currency), 420, rowY, 8, grayDark);
      drawText(page, height, helveticaBold, formatMoney(ligne.amount, factureRow.currency), width - 65, rowY, 9, nexusBlue, "right");
      rowY += rowHeight;
    }
    ...
    drawText(page, height, helveticaBold, formatMoney(factureRow.amount, factureRow.currency), width - 65, cursorY, 16, nexusOrange, "right");
    cursorY += 30;

    if (factureRow.due_date) {
      drawText(
        page,
        height,
        helvetica,
        `Échéance : ${new Date(factureRow.due_date).toLocaleDateString("fr-FR")}`,
        50,
        cursorY,
        9,
        grayDark
    ...
    return new NextResponse(Buffer.from(pdfBytes), {
      ...
      headers: {
        ...
        "Content-Disposition": `inline; filename="${sanitizeForPdf(factureRow.reference)}.pdf"`,
      },
    });
  } catch (err) {
