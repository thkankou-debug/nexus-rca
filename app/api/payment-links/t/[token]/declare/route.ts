...
// ============================================================================
// API : POST /api/payment-links/t/[token]/declare
// Migration 034 : déclaration par jeton public (public_token), non déductible,
// plutôt que par référence (séquentielle, donc énumérable). Clé service_role —
// toute la validation métier ci-dessous (méthode, n° transaction, expiration,
// statut déjà vérifié) est identique à l'ancienne route par référence
// (app/api/payment-links/[reference]/declare/route.ts, conservée pour la
// page /payer/[reference] tant qu'elle n'est pas neutralisée).
//
// Chemin .../t/[token] (et non .../[token]) : Next.js App Router exige que
// tous les segments dynamiques d'un même niveau partagent le même nom, et
// .../[reference] existe déjà à ce niveau.
// ============================================================================
...
export async function POST(
  ...
  console.log("===== [PAY-DECLARE-TOKEN] START =====");
  ...
  try {
    ...
    if (fetchError) {
      console.error("[PAY-DECLARE-TOKEN] fetch error:", JSON.stringify(fetchError));
    }

    if (fetchError || !paymentLink) {
      console.error("[PAY-DECLARE-TOKEN] Lien introuvable");
      return NextResponse.json({ error: "Lien introuvable" }, { status: 404 });
    }

    const reference = paymentLink.reference;

    // Refus si déjà vérifié : une déclaration ne doit jamais écraser une
    // vérification déjà faite par le staff.
    ...
    console.log("[PAY-DECLARE-TOKEN] Déclaration:", reference, "→", METHOD_LABELS[body.methode_choisie], "Tx:", numeroTransaction);
    ...
    if (updateError) {
      console.error("[PAY-DECLARE-TOKEN] Erreur update:", updateError);
      return NextResponse.json({ error: updateError.message }, { status: 500 });
    }

    console.log("[PAY-DECLARE-TOKEN] Statut → paiement_declare");
    ...
    console.log("[PAY-DECLARE-TOKEN] EMAIL STAFF START → ", NEXUS_STAFF_EMAIL);
    try {
      ...
      if (result.error) {
        console.error("[PAY-DECLARE-TOKEN] STAFF email échec primary:", JSON.stringify(result.error));
        ...
        if (!r2.error) {
          ...
          console.error("[PAY-DECLARE-TOKEN] STAFF email échec fallback:", JSON.stringify(r2.error));
        }
      } else {
        staffEmailSent = true;
      }
    } catch (e) {
      console.error("[PAY-DECLARE-TOKEN] STAFF email exception:", e instanceof Error ? e.message : e);
    }
    ...
    try {
      ...
      if (result.error) {
        console.error("[PAY-DECLARE-TOKEN] CLIENT email échec primary:", JSON.stringify(result.error));
        const r2 = await resend.emails.send({
          from: FROM_FALLBACK,
          to: paymentLink.client_email,
        ...
        if (!r2.error) {
          ...
          console.error("[PAY-DECLARE-TOKEN] CLIENT email échec fallback:", JSON.stringify(r2.error));
        }
      } else {
        clientEmailSent = true;
      }
    } catch (e) {
      console.error("[PAY-DECLARE-TOKEN] CLIENT email exception:", e instanceof Error ? e.message : e);
    }

    console.log("===== [PAY-DECLARE-TOKEN] END =====", { staff: staffEmailSent, client: clientEmailSent });
    ...
    console.error("[PAY-DECLARE-TOKEN] EXCEPTION:", err);
