...
| A1 | Tokens et fondations visuelles | **P** — tokens appliqués sur 8 fichiers publics + 7 générateurs PDF + **les 16 fichiers `app/services/*` (L12 lot 1, 13/09 : 2 666 occurrences requalifiées vers `brand`/`brand-hover`/`on-brand`, ombres `rgba(255,102,0,…)` → or, 0 `nexus-orange` restant sous `app/services`)**. **L12 lot 2 (13/09)** : pages publiques restantes + 78 composants partagés migrés (focus des formulaires → token `focus` bleu, scrollbar globale, utilitaires `text-gradient-*`, ombres `glow`/`glow-orange` et `mesh-gradient` de tailwind.config passés à l'or). Recompte : **1 246** occurrences restantes — toutes dans les dashboards hérités (`app/dashboard` 353 + `components/dashboard` 889), sauf le logo dessiné (`components/ui/Logo.tsx`, 3 — GELÉ par CLAUDE.md, décision Thierry requise) et 1 commentaire | 13/09 |
...
| P10 | Site public | **P** — `ServicesGrid.tsx`/`ServiceCard.tsx`/`lib/services.ts` corrigés 08/09 ; **les 12 pages `services/*` (+ `/services`, `[slug]`, devis assurance) migrées vers la palette or le 13/09 (L12 lot 1), tsc + build 0 erreur**. **Lot 2 livré (13/09)** : toutes les pages publiques et composants partagés migrés — le site public est à 0 orange (hors logo gelé, décision Thierry en attente). Reste de L12 : dashboards hérités (1 242 occ.), à traiter avec leur rhabillage ; gabarit factorisé = L13 | 13/09 |
