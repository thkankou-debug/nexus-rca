...
| A1 | Tokens et fondations visuelles | **P** — tokens appliqués sur 8 fichiers publics + 7 générateurs PDF + **les 16 fichiers `app/services/*` (L12 lot 1, 13/09 : 2 666 occurrences requalifiées vers `brand`/`brand-hover`/`on-brand`, ombres `rgba(255,102,0,…)` → or, 0 `nexus-orange` restant sous `app/services`)**. **L12 TERMINÉ en 3 lots (13/09)** : lot 1 pages services, lot 2 reste du site public + composants partagés, lot 3 dashboards hérités (157 fichiers, 1 242 occ. : mêmes règles + `shadow-nexus-orange` → `shadow-brand`, palette graphique RH et couleur d'événement calendrier `#FF6600` → `#B99760`). Recompte final : **3 occurrences** dans `components/ui/Logo.tsx` (logo dessiné — CONSERVÉ tel quel, tranché par Thierry le 13/09, voir docs/DECISIONS_AR.md) + 1 commentaire. **M11 « l'orange ne subsiste nulle part » : atteint, hors logo gelé.** Nota : les statuts des écrans hérités utilisent l'or là où ils utilisaient l'orange — le passage aux tokens `status.*` relève du rhabillage, pas de la couleur | 13/09 |
...
| P10 | Site public | **P** — `ServicesGrid.tsx`/`ServiceCard.tsx`/`lib/services.ts` corrigés 08/09 ; **les 12 pages `services/*` (+ `/services`, `[slug]`, devis assurance) migrées vers la palette or le 13/09 (L12 lot 1), tsc + build 0 erreur**. **Lots 2 et 3 livrés (13/09)** : site public ET dashboards hérités à 0 orange (hors logo, conservé tel quel — tranché par Thierry le 13/09). L12 clos ; gabarit factorisé = L13 | 13/09 |
