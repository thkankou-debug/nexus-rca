...
import {
  ...
  DOCUMENTS_OPTIONS,
  DOMAINES,
  ETABLISSEMENTS_CIBLES_OPTIONS,
  ETUDES_FORM_STORAGE_KEY,
  FRANCAIS_LEVELS,
  MOYENNES,
  NIVEAUX_ACTUELS,
  NIVEAUX_VISES,
...
const STEPS = [
  ...
  { id: 4, title: "Admission", icon: FileText },
  { id: 5, title: "Synthèse", icon: ShieldCheck },
] as const;

// ─── Composant principal ────────────────────────────────────────────────────

export function EtudesForm() {
  const router = useRouter();
  const supabase = createClient();
  const reduceMotion = useReducedMotion();

  const [screen, setScreen] = useState<Screen>("intro");
  const [form, setForm] = useState<EtudesFormData>(DEFAULT_ETUDES_FORM);
  ...
  useEffect(() => {
    ...
    try {
      const draft = localStorage.getItem(ETUDES_FORM_STORAGE_KEY);
      if (draft) {
        const parsed = JSON.parse(draft) as Partial<EtudesFormData>;
        setForm((f) => ({ ...f, ...parsed }));
      }
    } catch {}
  ...
  useEffect(() => {
    if (!hydrated.current) return;
    try {
      localStorage.setItem(ETUDES_FORM_STORAGE_KEY, JSON.stringify(form));
    } catch {}
  }, [form]);
  ...
  const update = <K extends keyof EtudesFormData>(
    key: K,
    value: EtudesFormData[K]
  ) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const focusPermis = form.admission_status === "admission_recue";
      const objet = focusPermis
        ? `Études Canada — Permis IRCC (${niveauViseLabel(form.niveau_vise)})`
        : `Études Canada — Admission ${niveauViseLabel(form.niveau_vise)} en ${domaineLabel(form.domaine)}`;
      ...
      try {
        localStorage.removeItem(ETUDES_FORM_STORAGE_KEY);
      } catch {}
    ...
    <div className="rounded-3xl border border-line bg-surface-elevated p-8 shadow-elev-2 sm:p-12">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <h2 className="mt-3 font-display text-display-md text-ink sm:text-display-lg">
          Étude de faisabilité — Admission & permis d&apos;études
        </h2>
        <p className="mx-auto mt-4 max-w-xl text-body-lg text-ink-muted">
          Environ 7 minutes. Vos réponses nous permettent d&apos;établir un
          bilan de faisabilité honnête de votre projet — qu&apos;il s&apos;agisse
          de structurer votre admission ou de sécuriser votre permis d&apos;études IRCC.
          Un conseiller Nexus reprend contact sous 24 heures ouvrées.
        </p>
...
function Step1({
  ...
  form: EtudesFormData;
  update: <K extends keyof EtudesFormData>(
    k: K,
    v: EtudesFormData[K]
  ) => void;
    ...
    >
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <Select
          ...
          onChange={(e) =>
            update("niveau_actuel", e.target.value as typeof form.niveau_actuel)
          }
      ...
      <div className="mt-6 space-y-3">
        ...
        {showLongRunway && (
          <Notice tone="info">
            ...
            préparation linguistique (TCF/IELTS) et une mise en conformité
            sereine du dossier.
          </Notice>
        )}
        {showQuebecHint && (
          <Notice tone="neutral">
            ...
            <strong>CAQ</strong> (Certificat d&apos;acceptation) en plus du
            permis d&apos;études IRCC. Nous prenons en charge les deux.
          </Notice>
        )}
        {showDoctoratHint && (
          <Notice tone="warn">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <RadioGroup
            ...
            onChange={(v) => update("tcf_passe", v as typeof form.tcf_passe)}
            options={[
      ...
      <div className="mt-6 space-y-3">
        {showLowGradeFlag && (
          <Notice tone="warn">
            ...
            canadiens ont des seuils d&apos;admission. Un conseiller senior
            étudiera les options réalistes pour votre profil avant tout
            engagement.
          </Notice>
        )}
        {showLanguageFlag && (
          <Notice tone="info">
            ...
            certification de français reconnue, l&apos;admission peut être
            limitée. Notre service{" "}
            <Link
              href="/services/tcf"
...
function Step4({
  ...
  const showAdmissionRecueHint = form.admission_status === "admission_recue";
  const showIndecisHint = form.etablissements_cibles === "indecis";

  return (
    <StepCard
      title="État d'avancement & motivation"
      subtitle="Où en êtes-vous dans la procédure, ancrage RCA et raisons du choix Canada."
    >
      <div className="space-y-6">
        <div>
          <p className="mb-2 text-caption font-semibold text-ink">
            Où en êtes-vous dans la procédure d&apos;admission ? *
          </p>
          <div className="grid gap-2 sm:grid-cols-2">
            {ADMISSION_STATUS_OPTIONS.map((opt) => (
              <RadioCard
                key={opt.value}
                checked={form.admission_status === opt.value}
                onChange={() => update("admission_status", opt.value)}
                label={opt.label}
        ...
        <Select
          label="Combien d'établissements ciblez-vous ? *"
          value={form.etablissements_cibles}
          onChange={(e) =>
            update(
              "etablissements_cibles",
              e.target.value as typeof form.etablissements_cibles
            )
          }
          error={errors.etablissements_cibles}
        >
          <option value="">Sélectionner…</option>
          {ETABLISSEMENTS_CIBLES_OPTIONS.map((opt) => (
        ...
        {showAdmissionRecueHint && (
          <Notice tone="info">
            <strong>Admission déjà reçue.</strong> Excellente nouvelle — nous
            concentrerons notre accompagnement sur le permis d&apos;études IRCC
            (et CAQ si Québec), la biométrie à Yaoundé et la préparation au
            départ.
          </Notice>
        )}
        {showIndecisHint && (
          <Notice tone="neutral">
            <strong>Pas encore décidé(e) ?</strong> Pas de problème — la
            cartographie d&apos;établissements alignés à votre profil fait
            partie de notre méthode. Un conseiller proposera 2 à 4 cibles
            réalistes après l&apos;analyse.
          </Notice>
        )}
      ...
      <div className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-2">
          <SummaryCard title="Votre projet">
            ...
            />
            <SummaryLine
              label="Établissements ciblés"
              value={etablissementsCiblesLabel(form.etablissements_cibles)}
            />
          </SummaryCard>
          ...
          <SummaryCard title="Votre profil">
            ...
            />
            <SummaryLine label="TCF Canada" value={form.tcf_passe || "—"} />
            <SummaryLine
              label="Admission"
              value={admissionStatusLabel(form.admission_status)}
            />
          </SummaryCard>
        </div>
        ...
        <div className="rounded-3xl border border-line bg-surface-elevated p-5">
          ...
          <div className="space-y-3">
            ...
            <ConsentCheckbox
              ...
              onChange={() => {}}
              disabled
              label="Je reconnais qu'aucune obtention d'admission ou de visa n'est garantie et que Nexus RCA fournit un service d'accompagnement professionnel."
            />
          </div>
        </div>
...
function RadioCard({
  ...
  return (
    ...
    >
      ...
      <span className="ml-7 text-caption text-ink-muted">{description}</span>
    </label>
  );
}
...
function buildSubmissionDescription(form: EtudesFormData): string {
  const fais = computeFaisabilite(form);
  const focusPermis = form.admission_status === "admission_recue";

  const lines = [
    `Demande Études Canada — soumise via formulaire dédié.`,
    `Focus : ${focusPermis ? "Permis d'études IRCC (admission déjà reçue)" : "Admission + permis d'études"}`,
    ``,
    `Projet :`,
    ...
    `- Établissements ciblés : ${etablissementsCiblesLabel(form.etablissements_cibles)}`,
    ``,
    `Profil :`,
    `- Niveau actuel : ${niveauActuelLabel(form.niveau_actuel)}`,
    `- Tranche d'âge : ${trancheAgeLabel(form.tranche_age)}`,
    ...
    ``,
    `Avancement & ancrage :`,
    `- État admission : ${admissionStatusLabel(form.admission_status)}`,
