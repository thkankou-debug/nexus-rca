...
const TONE_RING: Record<Tier["tone"], string> = {
  ...
  orange:
    "ring-brand/30 border-brand/40 from-brand/15",
...
export function TcfScoreCalculator() {
  ...
  return (
    <article
      className={`group relative overflow-hidden rounded-3xl border bg-gradient-to-br via-white/[0.04] to-white/[0.02] p-7 ring-1 backdrop-blur-xl shadow-[inset_0_1px_0_0_rgba(255,255,255,0.06),0_24px_48px_-16px_rgba(185,151,96,0.25)] sm:p-9 ${TONE_RING[tier.tone]}`}
    >
      <div
        aria-hidden
      ...
      <div className="relative">
        ...
        <div className="mt-7">
          <label
            htmlFor="tcf-score"
            className="mb-2 flex items-center justify-between text-[10px] font-bold uppercase tracking-[0.18em] text-brand"
          >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="relative h-6 w-full cursor-pointer appearance-none bg-transparent [&::-webkit-slider-thumb]:h-6 [&::-webkit-slider-thumb]:w-6 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-white [&::-webkit-slider-thumb]:bg-brand [&::-webkit-slider-thumb]:shadow-[0_8px_20px_-4px_rgba(185,151,96,0.7)] [&::-moz-range-thumb]:h-6 [&::-moz-range-thumb]:w-6 [&::-moz-range-thumb]:appearance-none [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border-2 [&::-moz-range-thumb]:border-white [&::-moz-range-thumb]:bg-brand"
            />
          </div>
            ...
            {TIERS.map((t, i) => (
              <button
                ...
                className={`rounded-lg border px-1 py-1.5 text-[9px] font-bold uppercase tracking-[0.10em] transition-all duration-200 backdrop-blur ${
                  i === idx
                    ? "border-brand/60 bg-brand/15 text-white shadow-[0_4px_14px_-4px_rgba(185,151,96,0.5)]"
                    : "border-white/10 bg-white/[0.03] text-white/60 hover:border-brand/30 hover:bg-white/[0.06] hover:text-white"
                }`}
              >
                NCLC {t.nclc}
              </button>
          ...
          <ul className="mt-3 space-y-2">
            {tier.uses.map((u) => {
              ...
              return (
                ...
                >
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-brand/10 text-brand ring-1 ring-brand/30 backdrop-blur">
