...
interface PublicHeroProps {
  ...
  /** Clé visuel (slug service) — image réelle, titres restent en texte */
  imageKey?: string;
}
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-nexus-blue-950 via-nexus-blue-900 to-nexus-blue-950 px-4 pt-16 pb-24 text-white sm:px-6 sm:pt-20 sm:pb-32 lg:px-8">
      ...
      <div
        className={
          imageKey
            ? "relative mx-auto grid max-w-6xl items-center gap-10 lg:grid-cols-2"
            : "relative mx-auto max-w-5xl text-center"
        }
      >
        <div className={imageKey ? "text-center lg:text-left" : undefined}>
          ...
          <p
            className={
              imageKey
                ? "mt-6 max-w-2xl text-base leading-relaxed text-slate-300 sm:text-lg lg:mx-0"
                : "mx-auto mt-6 max-w-2xl text-base leading-relaxed text-slate-300 sm:text-lg"
            }
          >
            {subtitle}
          </p>
          ...
          {(ctaPrimary || ctaSecondary) && (
            <div
              className={
                imageKey
                  ? "mt-8 flex flex-col items-stretch gap-3 sm:flex-row sm:justify-start"
                  : "mt-8 flex flex-col items-stretch gap-3 sm:flex-row sm:justify-center"
              }
            >
              {ctaPrimary && (
                <Link
        ...
        {imageKey && (
          <ServiceCover
            ...
            variant="hero"
            priority
            className="rounded-3xl ring-1 ring-white/10 shadow-[0_28px_60px_-24px_rgba(2,7,31,0.6)]"
          />
        )}
      </div>
    </section>
